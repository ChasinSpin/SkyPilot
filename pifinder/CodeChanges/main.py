...
def main(
    ...
    authedFirst = True
    try:
        with open(Path(utils.data_dir, 'pin.dat'), 'r') as pcode_fp:
            pcodeTmp = list(pcode_fp.readline()[0:-1])
            pcode = []
            for pt in pcodeTmp:
                pcode.append(int(pt))
        validSequence = []
    ...
    with StateManager() as manager:
        ...
        if not authed:
            menu_manager.message("Enter PIN", timeout = 3600)
        ...
        try:
            while True:
                ...
                if authed and authedFirst:
                    ...
                    menu_manager.message("Access Granted", timeout = 3)
                ...
                if keycode is not None:
                    ...
                    if not authed:
                        if keycode < 10:
                            validSequence.append(keycode)
                            print(validSequence)
                            print(pcode[0:len(validSequence)])
                            if validSequence == pcode[0:len(validSequence)]:
                                    if validSequence == pcode:
                                        authed = True
                            else:
                                validSequence = []
                        else:
                    ...
                    if original_power_state > 0 and authed:
                        if keycode > 99:
            ...
            if posserver_process is not None:
