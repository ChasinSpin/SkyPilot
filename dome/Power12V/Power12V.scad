// Drag and drop STL into Bambu Studio with settings as follows.  Note it's normal for the stars to be above the case:
//
// ASA Black and ASA Green for Star Infill
// Wall loops: 4
// Bottom surface pattern: Monotonic Line
// Bottom shell layers: 9
// Sparse infill density: 60%
// Support: Disbaled
// Smooth PEI Surface with Liquid Glue
// 0.20mm Standard @BBL X1C Layer Height

// After import of the STL:
//      Right Click Model -> Split -> To Parts
//      Click "Objects" (Next to Process)
//      Select Last Item (NfcOutdoorCase.stl_4) - the hovering stars, right click color and change to Green
//      Click another object for the last change to take
//      Click on NfcOutdoorCase.stl_4 again, and click the Translate button
//      Enter -20 for Z
//      Click Global (Next to Process)
//      Slice and check only 1st 2 layers contain color for the stars
...
MountHolderStyle          = 1;      // [1:Style 1, 2: Style 2, 3: Style 3, 4 : Style 4, 5 : Style 5]
...
translate([X_ObjectPosition,0,0])
    rotate([0,0,0])
    {
        difference()
        {
...
translate([-X_ObjectPosition,0,Z_TopHigh+0.03]) rotate([0,Y_TopRotation,0])
{
    difference(){
        union(){
            BodyTop();
            ...
            translate( [0, 25, 3.0] )
                cylinder(d = 18.0, h = 20.0 + 0.01);
        }
        // **** Add your top case cuts here ****
        translate( [0, 25, -0.01] )
        {
            cylinder(d = 10.4, h = 6.32);
            translate( [0, 0, 3.0] )
                cylinder(d = 11.85, h = 20.0 + 0.03, $fn = 6);
        }
...
module writeText()
{
    ...
    translate( [0, 0, textDepth - manifoldCorrection] )
    {    
        translate( [7, 0, 0] )
            rotate( [180, 0, 90] )
                linear_extrude(textDepth)
                    text("12V DC", halign = "center", size = 10);
           
        translate( [-15, 0, 0] )
            rotate( [180, 0, 90] )
                linear_extrude(textDepth)
                    text("5 Amp Max.", halign = "center", size = 10);
    }
}
...
module BodyQuarterBottom (Caselength,CaseWidth,CutFromTop,CaseRoundingRadius,SideWallThickness) {
    difference(){
        union(){
            color("Red")BodyQuarter(Caselength,CaseWidth,CutFromTop,CaseRoundingRadius,SideWallThickness);
        ...
        if ( useInsert )    
            {translate(ScrewCornerPos) translate([0,0,CutFromTop+0.01]) InsertCut(CutFromTop,insertDepth,insertDiameter);}
...
module InsertCut(TotalHigh, High, Dia)
{
    translate([0,0,-High] )
        cylinder(h=High,d=Dia,center = false);
    
    translate([0,0,-(TotalHigh-SideWallThickness)/2])
        cylinder(h=TotalHigh-SideWallThickness,d=ScrewHoleDia - 0.5,center = true);
}
